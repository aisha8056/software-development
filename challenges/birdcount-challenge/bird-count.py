...
def get_bird_by_key(bird_list, key):  # Loop through each bird dictionary in the list
    for bird in bird_list:
        if bird['key'] == key:  # If the bird's "key" value matches the given "key", return the entire bird dictionary
            return bird
